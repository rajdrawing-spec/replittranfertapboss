...
export interface TemplatedWithPriority extends TaskTemplate {
  effectivePriority: string;
}

export function buildBatchPrompt(
  employees: EmployeeProfile[],
  templates: TemplatedWithPriority[],
  customPromptTemplate?: string | null,
): string {
  const lines: string[] = [];
  lines.push("Employees in this batch:");
  for (const emp of employees) {
    lines.push(`- ID ${emp.id}: ${emp.firstName} ${emp.lastName} | Department: ${emp.department} | Role: ${emp.designation} | Skill: ${emp.skillLevel || "unspecified"} | Working hours: ${emp.workingHours || "unspecified"} | Current project: ${emp.currentProject || "none"} | Status: ${emp.status}`);
  }
  lines.push("\nTemplates to customize (priority may be elevated by project importance):");
  for (const t of templates) {
    const priority = t.effectivePriority || t.priority;
    lines.push(`- ID ${t.id}: [${priority}] ${t.titleTemplate} | ${t.descriptionTemplate} | Recurrence: ${t.recurrence} | Est. minutes: ${t.estimatedMinutes || "unspecified"}`);
  }
...
export function getSystemPrompt(): string {
  return `You are a task-assistant AI for a business operating system.
...
- Respect the employee's working hours, skill level, and current project. Do not assign high-load tasks to junior staff without guidance.
- Respect project priority: if the template title or description relates to a high/critical project, raise the priority accordingly.
