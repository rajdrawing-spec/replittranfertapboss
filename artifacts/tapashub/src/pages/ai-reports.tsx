...
const TYPE_COLORS: Record<string, string> = {
  ...
  weekly:    "bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-500/20",
  ...
  annual:    "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20",
  manual:    "bg-slate-500/10 text-slate-700 dark:text-slate-400 border-slate-500/20",
}
const SEVERITY_COLORS: Record<string, string> = {
  critical: "text-red-700 dark:text-red-400 bg-red-500/10 border-red-500/20",
  high:     "text-orange-700 dark:text-orange-400 bg-orange-500/10 border-orange-500/20",
  medium:   "text-yellow-700 dark:text-yellow-400 bg-yellow-500/10 border-yellow-500/20",
  low:      "text-blue-700 dark:text-blue-400 bg-blue-500/10 border-blue-500/20",
}
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        ...
        isGreen  && "text-green-700 dark:text-green-400",
      ...
      {delta && (
        <div className={cn("text-xs mt-1 font-medium", delta.startsWith("+") ? "text-green-700 dark:text-green-400" : "text-red-700 dark:text-red-400")}>
          {delta}
        </div>
      )}
    </div>
    ...
    <div className="fixed inset-0 z-50 bg-background overflow-y-auto">
      <div className="max-w-5xl mx-auto p-6 space-y-6" id="report-print-root">
        ...
        {(content.risks.length > 0 || content.opportunities.length > 0) && (
          <section>
            <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3 flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-amber-700 dark:text-amber-400" /> Risks & Opportunities
            </h2>
                    ...
                    {content.opportunities.map((o, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <Badge variant="outline" className={cn("text-[9px] shrink-0 mt-0.5",
                          o.impact === "high" ? "text-green-700 dark:text-green-400 bg-green-500/10 border-green-500/20" :
                          o.impact === "medium" ? "text-blue-700 dark:text-blue-400 bg-blue-500/10 border-blue-500/20" :
    ...
    <div className="flex items-center gap-3 p-3 rounded-lg border hover:bg-muted/20 transition-colors">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <Badge variant="outline" className={cn("text-[10px]",
          isReady ? "bg-green-500/10 text-green-700 dark:text-green-400 border-green-500/20" :
          report.status === "generating" ? "bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-500/20" :
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {generateNow.isSuccess && (
        <div className="text-xs text-green-700 dark:text-green-400 flex items-center gap-2 bg-green-500/5 border border-green-500/15 rounded-lg px-4 py-2">
          ✓ Report queued — it will appear in the list below momentarily.
        </div>
      )}
      {generateNow.isError && (
        <div className="text-xs text-red-700 dark:text-red-400 flex items-center gap-2 bg-red-500/5 border border-red-500/15 rounded-lg px-4 py-2">
